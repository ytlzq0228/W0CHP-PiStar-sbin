...
if ! grep -q 'Version 3.9,' /usr/local/sbin/pistar-update; then
    ...
    sudo env GIT_HTTP_CONNECT_TIMEOUT="10" env GIT_HTTP_USER_AGENT="W0CHP-PS-D_Updste_Check (temp_force)" git --work-tree=/usr/local/sbin --git-dir=/usr/local/sbin/.git pull origin master
    ...
    sudo git reset --hard
    sudo env GIT_HTTP_CONNECT_TIMEOUT="10" env GIT_HTTP_USER_AGENT="W0CHP-PS-D_Updste_Check (temp_force)" git --work-tree=/var/www/dashboard --git-dir=/var/www/dashboard/.git pull origin ${gitBranch}
